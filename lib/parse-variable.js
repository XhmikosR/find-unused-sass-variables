...
const CSS_VAR_REF_RE = /var\((--[\w-]+)/g;

// Adds every variable reference found in text directly to usages
function addVariableRefs(text, usages, cssVariables) {
  if (!text) return;
  ...
  if (cssVariables) {
    for (const [, name] of text.matchAll(CSS_VAR_REF_RE)) {
      usages.add(name);
    }
  }
}
...
function addAtruleUsages(node, usages, cssVariables) {
  switch (node.name) {
    case 'mixin':
    case 'function': {
    ...
    default: {
      addVariableRefs(node.params, usages, cssVariables);
    }
  }
}

// ctx = { fileName, ignoreList, cssVariables }
// Returns [variables, usages, newEnabled]
function collectFromNode(node, enabled, ctx) {
  ...
  const { fileName, ignoreList, cssVariables } = ctx;
  const usages = new Set();
  ...
  if (node.type === 'decl') {
    ...
    if (node.prop.startsWith('$') && !ignoreList.includes(node.prop) && enabled) {
    ...
    } else if (cssVariables && node.prop.startsWith('--') && !node.prop.includes('#{') && !ignoreList.includes(node.prop) && enabled) {
      variables.push({
        name: node.prop,
        file: fileName,
        line: node.source.start.line
      ...
      addVariableRefs(node.prop, usages, cssVariables);
    }

    addVariableRefs(node.value, usages, cssVariables);
    return [variables, usages, enabled];
  }

  if (node.type === 'atrule') {
    addAtruleUsages(node, usages, cssVariables);
  } else if (node.type === 'rule') {
    addVariableRefs(node.selector, usages, cssVariables);
  }
  ...
  if (node.nodes) {
    ...
    for (const child of node.nodes) {
      const [childVariables, childUsages, newEnabled] = collectFromNode(child, currentEnabled, ctx);
...
export function parse(fileName, scssString, ignoreList, cssVariables = false) {
  const ctx = { fileName, ignoreList, cssVariables };
  let enabled = true;
  const variables = [];
  const usages = new Set();

  for (const node of scssParse(scssString).nodes) {
    const [nodeVariables, nodeUsages, newEnabled] = collectFromNode(node, enabled, ctx);
